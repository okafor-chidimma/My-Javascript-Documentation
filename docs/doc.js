...
when the await keyword is used, it waits until the Promise resolves or rejects. this means that "await" keyword can only be used on a function that returns a promise object, so it waits for the promise object to resolve or reject

HOW DO I ACCESS THE REAL RESOLVED VALUE?
there are 2ways we can do that
1. by chaining a .then() to the async function. as shown below

    myAsyncFunc().then((resp)=>{
